...
search_ares (unsigned char *haystack, int packet_len, int head_len)
{
	...
	{
		const __u16 size=packet_len - head_len;
		switch (t[2])
		{
			case 0x5a:
				/* ares connect */
...
int match(struct iphdr *ip)
{
    ...
    haystack=(unsigned char *)ip+(ip->ihl*4);		/*haystack = packet data*/
	...
	case IPPROTO_TCP:		/*what to do with a TCP packet*/
	{
		...
		if (hlen > matchlist[i].packet_len) {
			    ...
			    if (p2p_result) 
			    {
				//printf("IPP2P.debug:TCP-match: %i from: %u.%u.%u.%u:%i to: %u.%u.%u.%u:%i Length: %i\n", 
	...
	case IPPROTO_UDP:		/*what to do with an UDP packet*/
	{
		...
		if (hlen > udp_list[i].packet_len) {
			    ...
			    if (p2p_result){
				//printf("IPP2P.debug:UDP-match: %i from: %u.%u.%u.%u:%i to: %u.%u.%u.%u:%i Length: %i\n", 
